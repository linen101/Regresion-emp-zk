#include "secret_sum_range_check.hpp"
#include "host_ip.hpp"
...
vector<data_entry> create_random_dataset(vector<int64_t> &precomputed_dataset, vector<IntFp> &zk_precomputed_dataset) {
...
int main(int argc, char** argv) {
	...
	std::cout << std::endl << "------------ ";
	std::cout << "ZKP secret sum, range check";
	...
	for(int i = 0; i < dataset.size(); i++) {
		secret_range_check(dataset[i].inp, dataset[i].zk_inp, 1, range_len, zk_zero_checking, zk_l);
	}
